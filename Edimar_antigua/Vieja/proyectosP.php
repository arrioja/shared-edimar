            ...
            <td><table width="100%"  border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td height="35"><span class="Titulo_II">
                  <?php  echo utf8_decode($rowEdimar["Nombre"]." ".$rowEdimar["Apellido"]);?>
                </span></td>
              </tr>
              <tr>
                  ...
                  <tr align="center" bgcolor="#6699CC">
                    <td height="35" colspan="2" class="Titulo_I">
                      <?php  if ((isset($_GET["Id"]))&& ($_GET["Id"]=='_Ing'))
			                  { echo "Research Programs"; }else {echo "Proyectos de Investigaci&oacute;n";};?>                   </td>
                  </tr>
				 <?php $i=1;
					$query_reg = "SELECT Cod_pe, Cod, Titulo, Descarga FROM proyecto inner join per_proy on Cod=Cod_Pro and Cod_Pe='".$_GET["Cod"]."'";
                    $Proyecto = mysql_query($query_reg, $conexion) or die(mysql_error());
					?>
					<?php while ($regProyecto = mysql_fetch_assoc($Proyecto)) {?>
                  <tr>
                    <td width="89%" height="40" class="borde_SupI"><p class="Parrafo"><?php echo $regProyecto["Titulo"]; ?></p></td>
                    <td width="11%" align="center" class="borde_Sup"><a href=<?php echo "Proyectos/".$regProyecto["Descarga"]; ?>><img border="0" name=<?php echo "pdf".$i; ?> src="Imagenes/pdf.gif" width="35" height="33"></a></td>
                  </tr>
				  <?php }?>
                </table>
