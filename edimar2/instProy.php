...
if (isset($_POST['boton']))  {
  ...
  if  ($_POST['boton']=="Aceptar") {
	...
	if ((trim($_POST['Participantes'])!="")&&(trim($_POST['Titulo'])!="")&&(trim($_POST['Tip'])!="")&&(trim($_POST['Objetivos'])!="")){
	 ...
	 if ($_GET["Op"]=='M'){
       ...
       $insertvend = sprintf("update proyecto, edimar, departamento set proyecto.Fase=%s, proyecto.Titulo=%s, proyecto.Objetivos=%s, proyecto.Participantes=%s, proyecto.Dpto=%s, proyecto.Linea=%s,proyecto.Tipo=%s, edimar.FechaA=%s, departamento.FechaA=%s where departamento.Cod=proyecto.Dpto and departamento.Idioma=edimar.Idioma and proyecto.Idioma=departamento.Idioma and  proyecto.Cod=%s",
	  ...
	  if ($_GET["CodD"]!=''){$Dpt=$_GET["CodD"];}else{$Dpt=$_POST['Dpto'];}; 
	    $insertvend = sprintf("INSERT INTO proyecto (Titulo, Fase, Objetivos, Participantes,Dpto,Linea, Tipo, Descarga,Permiso,Idioma) VALUES (%s, %s, %s,%s, %s, %s, %s, %s, %s, %s)",
                       GetSQLValueString($_POST['Titulo'], "text"),
					   GetSQLValueString($_POST['Fase'], "text"),
					   GetSQLValueString($_POST['Objetivos'], "text"),
					   GetSQLValueString($_POST['Participantes'], "text"),
	 ...
	 $insertvend = sprintf("update edimar,departamento set edimar.FechaA=%s, departamento.FechaA=%s where departamento.Cod=%s and
	                        departamento.Idioma=edimar.Idioma and departamento.Idioma=%s",
                       GetSQLValueString(date('Y/m/d'), "text"),
					   GetSQLValueString(date('Y/m/d'), "text"),
					   GetSQLValueString($Dpt, "text"),
					   GetSQLValueString($id, "text"));
  ...
  if ($_GET["Op"]=='M'){
  $query_reg_most = "SELECT proyecto.Cod, Nombre AS Dpto, departamento.Cod AS CodDpto, proyecto.Titulo, Fase, Objetivos, Participantes,                       Tipo, linea_invest.Cod AS Codlinea, linea_invest.Titulo AS linea FROM proyecto INNER JOIN departamento
                      on Dpto=departamento.Cod and departamento.Idioma='".$id."' and proyecto.Cod='".$_GET["Cod"]."'
					  INNER JOIN linea_invest ON linea_invest.Cod = linea";
  ...
  $Dpto1=$rowProy["Dpto"];$CodDpto=$rowProy["CodDpto"];$Codlinea=$rowProy["Codlinea"];$Linea1=$rowProy["linea"];
                        ...
                        <tr >
                          <td align="center" colspan="2" >
					        ...
					        ?>
                              ...
                              <tr>
                                <td height="40">*Linea</td>
                                <td align="left">
								<?php  
					           $query="SELECT Cod, Titulo FROM linea_invest where Idioma='".$id."'";
                               $reg_linea=mysql_query($query, $conexion) or die(mysql_error());
					        ?><select name="linea" id="linea">
                                  <?php  if ($_GET["Op"]=='M'){?>
                                  <option value="<?php echo $Codlinea?>"><?php echo $Linea1; ?> </option>
                                  <?php } $i=0;
								   while ($row_Linea = mysql_fetch_assoc($reg_linea)){ $i++; ?>
                                  <option value="<?php echo $row_Linea['Cod']?>"><?php echo $row_Linea['Titulo']; ?> </option>
                                  <?php }   ?>
                                </select></td>
                                <td align="left">&nbsp;</td>
                                <td align="left">&nbsp;</td>
