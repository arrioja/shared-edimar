  ...
  if  ($_POST['boton']=="Aceptar") {
	...
	if (trim($_POST['Descrip'])!=""){
	 ...
	 if ($_GET["Op"]=='M'){
       ...
       $insertvend = sprintf("update publicacion, edimar,departamento set  publicacion.Descrip=%s, publicacion.Dpto=%s, edimar.FechaA=%s, 
	                          departamento.FechaA=%s where departamento.Cod=publicacion.Dpto and departamento.Idioma=edimar.Idioma and 
							  publicacion.Idioma=departamento.Idioma and publicacion.Cod=%s",
	  ...
	  if ($nom_arch!=''){		copy($arch, "Publicaciones/".$nom_arch);}$Dpt='';
