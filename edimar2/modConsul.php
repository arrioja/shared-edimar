...
if (isset($_POST['boton']))  {
  ...
  if  ($_POST['boton']=="Aceptar") {
    //modificar  noticias
	if ((trim($_POST['Parraf1'])!="")&&(trim($_POST['Parraf2'])!="")){$err=0;
	mysql_query("BEGIN",$conexion); 
       $insertvend = sprintf("update consultoria, edimar set consultoria.Descrip=%s,Contactos=%s, FechaA=%s  where consultoria.Cod=%s and consultoria.Idioma=edimar.Idioma",
  ...
  if ($vacios==0){header("Location: Consultoria.php?".$UR."&Id=".$_GET["Id"]."&Usr=".$_GET["Usr"]);}
}else{
  $query_reg_most = "SELECT * FROM  consultoria where Idioma='".$id."'";
  $reg_most = mysql_query($query_reg_most, $conexion) or die(mysql_error());
  $rowEdimar = mysql_fetch_assoc($reg_most);
  $totalRows_reg_most = mysql_num_rows($reg_most);
  $Cod=$rowEdimar['Cod'];  $Parraf1=$rowEdimar['Descrip']; $Parraf2=$rowEdimar['Contactos']; 
          ...
          <tr>
            <td><br>
				...
				<form name="form2" method="post" action="">
                  <tr>
                    <td height="40" colspan="2" class="Titulo_II">*Decripci&oacute;n: </td>
                  </tr>
                  <tr>
                    <td colspan="2" class="Parrafo"><textarea name="Parraf1" cols="80" rows="20" id="Parraf1"><?php echo $Parraf1; ?>
                      </textarea>                       </td>
                    </tr>
                  <tr>
                    <td height="40" colspan="2"  class="Titulo_II">*Contactos:</td>
                  <tr>
